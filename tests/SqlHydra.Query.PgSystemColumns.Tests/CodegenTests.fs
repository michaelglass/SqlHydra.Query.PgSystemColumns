...
let ``every system column is read-only`` () =
    // PostgreSQL refuses `INSERT INTO t (xmin)` and `SET xmin = ...` outright. In SqlHydra
    // 5.0 `IsReadOnly` splits the emitted type: the read record carries the column and the
    // companion `{table}_write` record does not, so it cannot reach a statement at all.
